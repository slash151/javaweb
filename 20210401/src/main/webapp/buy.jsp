...
<html>
		...
		<div class="buy">
			<form action="purchase succeed.jsp" method="get">
				...
				<div class="buy-info">
					...
					<hr class="buy-info-hr" /><br>
					<!-- 购买按钮 -->
					<button type="submit" class="btn btn-primary btnadpt submit" style="font-size: 20px;border-radius: 7px;">Buy</button>
	...
	<script>
		...
		if (params.length == 1) {
			//读取json文件，校验
			$.getJSON('JSON/iphone.json', function(infos) {
				var id = 0;
				//id和json中的数组循环比较，找到该id的产品
				for (var i = 0; i < infos.iphone.length; i++) {
					if (params == infos.iphone[i].pid) {
						id = i;
						checkout = true;
					}
				}
				//如果找到，就从json传值
				if (checkout) {
					var pimg = infos.iphone[id].pimg;
					var img = '<img src="' + pimg + '">';
					//在对应的元素中添加元素
					$('.buy-img').append(img);
					localStorage.setItem("img", img);
					var name = infos.iphone[id].pname;
					$('.buy-info-name').text(name);
					localStorage.setItem("name", name);
					var describe = infos.iphone[id].pinfos;
					$('.buy-info-describe').text(describe);
					var price = infos.iphone[id].price;
					$('.buy-info-price').text(price);
					var pars = infos.iphone[id].color.split("+");
					var li = $('.buy-info-color').find('li');
					for (k = 0; k < pars.length; k++) {
						li.eq(k).find("a > span").text(pars[k]);
					}
					var para = infos.iphone[id].version.split("#");
			...
			$.getJSON('JSON/iPad.json', function(infos) {
				var id = 0;
				for (var i = 0; i < infos.iPad.length; i++) {
					if (params == infos.iPad[i].pid) {
						id = i;
						checkout = true;
					}
				}
				if (checkout) {
					var pimg = infos.iPad[id].pimg;
					var img = '<img src="' + pimg + '">';
					$('.buy-img').append(img);
					localStorage.setItem("img", img);
					var name = infos.iPad[id].pname;
					$('.buy-info-name').text(name);
					localStorage.setItem("name", name);
					var describe = infos.iPad[id].pinfos;
					$('.buy-info-describe').text(describe);
					var price = infos.iPad[id].price;
					$('.buy-info-price').text(price);
					var pars = infos.iPad[id].color.split("+");
					var li = $('.buy-info-color').find('li');
					for (k = 0; k < pars.length; k++) {
						li.eq(k).find("a > span").text(pars[k]);
					}
					var para = infos.iPad[id].version.split("#");
					var li = $('.buy-info-capacity').find('li');
					for (k = 0; k < para.length; k++) {
						li.eq(k).find("a").text(para[k]);
					}
			...
			$.getJSON('JSON/Mac.json', function(infos) {
				var id = 0;
				for (var i = 0; i < infos.Mac.length; i++) {
					if (params == infos.Mac[i].pid) {
						id = i;
						checkout = true;
					}
				}
				if (checkout) {
					var pimg = infos.Mac[id].pimg;
					var img = '<img src="' + pimg + '">';
					$('.buy-img').append(img);
					localStorage.setItem("img", img);
					var name = infos.Mac[id].pname;
					$('.buy-info-name').text(name);
					localStorage.setItem("name", name);
					var describe = infos.Mac[id].pinfos;
					$('.buy-info-describe').text(describe);
					var price = infos.Mac[id].price;
					$('.buy-info-price').text(price);
					var pars = infos.Mac[id].color.split("+");
					var li = $('.buy-info-color').find('li');
					for (k = 0; k < pars.length; k++) {
						li.eq(k).find("a > span").text(pars[k]);
					}
					var para = infos.Mac[id].version.split("#");
					var li = $('.buy-info-capacity').find('li');
					for (k = 0; k < para.length; k++) {
						li.eq(k).find("a").text(para[k]);
					}
